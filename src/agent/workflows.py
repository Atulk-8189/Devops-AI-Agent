...
AKS_CLUSTER_HEALTH_SYSTEM_PROMPT = (
    ...
    "KUBERNETES NODE CONDITION SEMANTICS: "
    "Ready=True indicates normal node readiness. For node pressure conditions (MemoryPressure, DiskPressure, "
    "PIDPressure), status=False is the normal/healthy state meaning NO pressure exists on the node. "
    "status=True indicates active pressure. You MUST NOT interpret status=False pressure conditions as active "
    "problems, anomalies, or concerns. An active pressure problem exists ONLY when its status is 'True'. "
